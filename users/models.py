...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, username, phone_number, email, password, is_staff, is_superuser, **extra_fields):
        """
        Creates and saves a User with the given username, email ans password.
        """
        now = timezone.now()
        if not username:
    ...
    def create_user(self, username=None, phone_number=None, email=None, password=None, **extra_fields):
        if username is None:
            ...
            if phone_number:
                username = random.choice('abcdefghilklmnopqrstuvwxyz') + str(phone_number)[-7:]
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(_('username'), max_length=50, unique=True,
                                help_text=_('Required. 30 characters or fewer starting with a letter, letters, digital'),
                                validators=[
                                    validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                                                              _('Enter a valid username starting with a-z.This value may contain only letters, number and underscore characters.'))
                                ],
                                error_messages={
    ...
    def get_full_name(self):
        """
        Returns the first name plus the last name, whith a space in between.
        """
    ...
    def is_loggedin_user(self):
        """
        Returns True if user has acttualy logged in with valid credentials.
        """
...
class Device(models.Model):
    ...
    #    _('Notifaction Token'), max_length=200, blank=True,
    ...
    device_type = models.PositiveBigIntegerField(choices=DEVICE_TYPE_CHOICES, default=WEB)
